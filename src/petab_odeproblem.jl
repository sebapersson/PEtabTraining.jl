function _PEtabODEProblem(model::PEtabModel, prob_original::PEtabODEProblem)::PEtabODEProblem
    ...
    prob = PEtabODEProblem(model; odesolver = solver, odesolver_gradient = solver_gradient, ss_solver = ss_solver, ss_solver_gradient = ss_solver_gradient, gradient_method = gradient_method, hessian_method = hessian_method, sensealg = sensealg, reuse_sensitivities = reuse_sensitivities)
    ...
    prob_to_original = [findfirst(x -> x == name, prob.xnames) for name in prob_original.xnames]
    original_to_prob = [findfirst(x -> x == name, prob_original.xnames) for name in prob.xnames]
    ...
    return PEtabODEProblem(nllh, chi2, grad!, grad, hess!, hess, hess!, hess, nllh_grad,
        prior, grad_prior, hess_prior, simulated_values, residuals, prob.probinfo,
...
function _get_simulated_values(prob::PEtabODEProblem, original_to_prob::Vector{Int64})::Function
...
function _get_grad(prob::PEtabODEProblem, original_to_prob::Vector{Int64}, prob_to_original::Vector{Int64})::Function
    grad = let _original_to_prob = original_to_prob, _prob_to_original = prob_to_original, _prob = prob
        (x) -> begin
            _x = x[_original_to_prob]
            g = _prob.grad(_x)
...
function _get_nllh_grad(prob::PEtabODEProblem, original_to_prob::Vector{Int64}, prob_to_original::Vector{Int64})::Function
    nllh_grad = let _original_to_prob = original_to_prob, _prob_to_original = prob_to_original, _prob = prob
        (x) -> begin
            _x = x[_original_to_prob]
...
function _get_grad_prior(prob::PEtabODEProblem, original_to_prob::Vector{Int64}, prob_to_original::Vector{Int64})::Function
    grad_prior = let _original_to_prob = original_to_prob, _prob_to_original = prob_to_original, _prob = prob
        (g, x) -> begin
            _x = x[_original_to_prob]
...
function _get_hess(prob::PEtabODEProblem, original_to_prob::Vector{Int64}, prob_to_original::Vector{Int64})::Function
    hess = let _original_to_prob = original_to_prob, _prob_to_original = prob_to_original, _prob = prob
        (x) -> begin
            _x = x[_original_to_prob]
            H = _prob.hess(_x)
            H_out = similar(H)
...
function _get_hess_prior(prob::PEtabODEProblem, original_to_prob::Vector{Int64}, prob_to_original::Vector{Int64})::Function
    hess_prior = let _original_to_prob = original_to_prob, _prob_to_original = prob_to_original, _prob = prob
        (H_out, x) -> begin
            _x = x[_original_to_prob]
...
function _map_hessian!(H_original::T, H_prob::T, prob_to_original::Vector{Int64})::Nothing where T <: Matrix{<:AbstractFloat}
