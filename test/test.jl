...
function test_split_uniform(model_id, n_stages)
    path_yaml = joinpath(@__DIR__, "published_models", model_id, "$(model_id).yaml")
    petab_prob = PEtabModel(path_yaml) |>
        PEtabODEProblem
    ...
    unique_t = measurements_df.time |> sort |> unique
    ...
    for i in 1:n_stages
        measurements_tmp_df = measurements_df[measurements_df[!, :time] .≤ maximum(time_chunks[i]), :]
        CSV.write(petab_prob.model_info.model.paths[:measurements], measurements_tmp_df, delim = '\t')
        petab_prob_ref = PEtabModel(path_yaml) |>
            PEtabODEProblem
        CSV.write(petab_prob.model_info.model.paths[:measurements], measurements_df, delim = '\t')
...
@testset "Uniform splitting" begin
    for n_stages in [2, 3, 5]
        test_split_uniform("Boehm_JProteomeRes2014", n_stages)
    end
    for n_stages in [3, 4, 6]
        test_split_uniform("Weber_BMC2015", n_stages)
    end
    for n_stages in [2, 7]
        test_split_uniform("Bachmann_MSB2011", n_stages)
    end
end
